...
export const getEquipes = async (): Promise<Equipe[]> => {
    const response = await axios.get(`${BASE_URL}/equipes`, {
        headers: {
...
export const getEquipeById = async (idEquipe: string): Promise<Equipe> => {
    const response = await axios.get(`${BASE_URL}/equipes/${idEquipe}`, {
        headers: {
            Authorization: `Bearer ${BEARER_TOKEN}`,
        },
    });
